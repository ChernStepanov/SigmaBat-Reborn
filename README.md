...
SigmaBat Reborn создаёт `.bat`-лоадеры для managed EXE, DLL и shellcode, а затем передаёт выполнение в PowerShell. Для managed EXE он загружает сборку в память, находит точку входа и вызывает её. Для DLL он определяет, является ли файл managed или native, проверяет наличие нужного символа и затем вызывает его. Для shellcode полезная нагрузка временно собирается в переменной окружения лоадера, помещается в исполняемую память и стартует в отдельном потоке.
...
- Поддерживает managed EXE
- Поддерживает managed и native DLL
- Поддерживает шеллкод
- Вызывает символ по имени для DLL
- Вызывает точку входа для managed EXE
- Проверяет наличие символа и entry point до генерации лоадера
- Сохраняет необязательную batch-обфускацию
- Поддерживает `--no-obf` для вывода без обфускации
- Выводит короткие статусные сообщения для shellcode-режима
...
Shellcode:
- Читается из base64-блоба, временно собранного в переменной окружения лоадера
- Выделяется исполняемая память и стартует отдельный поток
- Выводит короткие статусные сообщения в консоль
- Предназначен для байтов shellcode, а не для managed-сборок

### Поток работы

1. Считать байты входного файла с диска.
2. Определить, managed это EXE, managed DLL или native DLL.
...
5. Сформировать `.bat`, который запускает PowerShell.
6. Вызвать полезную нагрузку во время выполнения.
7. При необходимости применить необязательную обфускацию.
...
- Python 3
- PowerShell
- Managed EXE, DLL или shellcode-блоб с совместимой полезной нагрузкой

### Примечания

- Если функция или точка входа отсутствует, генератор остановится до записи лоадера.
...
- Shellcode-лоадеры выводят короткие сообщения о ходе запуска, чтобы было видно начало и завершение выполнения.
